 ...
 *  Usage: 	Construct can_api with can_object_t static object pointer. can_object_t
...
namespace hal::can {
	...
	*/
	...
	using CanIrqHandlerType = util::delegate<void(CanIrqType, class can_api*, int32_t handle)>;

	// TODO delete all copy constructors and operators
	class can_api {
	public:
		/*
		* can_object_t resource to use
		*/
		can_api() = delete;
		explicit can_api(can_object_t* obj);
		virtual ~can_api();
		...
		*/
		...
		/*!
		* @brief Read message from handle, or from first full mb. This should be safe to call during an 
		* interrupt.
		*
