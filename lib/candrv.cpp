...
struct MB_info::Frame_data {
	...
	volatile _flexcan_frame fsl_frame;
...
void MB_info::do_callback() {
	if (completed) {
		completed = false;
		if (!callback.isNull())
			callback(const_cast<uint8_t*>(&frame_data->fsl_frame.dataByte0), frame_data->fsl_frame.length, bus);
	}
}
...
bool MB_info::set_frame_data_tx(uint32_t id,
	...
	if(!remote) {
		uint8_t* frame_data = &frame.dataByte0;
...
bool MB_info::set_frame_data_rx(uint32_t id, uint8_t dlc, bool remote,
	...
	flexcan_mb_transfer_t transfer = {};
	transfer.mbIdx = mb_id;
	transfer.frame = const_cast<_flexcan_frame*>(&frame_data->fsl_frame);
